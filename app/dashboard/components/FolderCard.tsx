...
export default function FolderCard({
  ...
  return (
    <div
      ...
      className={`group relative flex cursor-pointer rounded-xl border border-zinc-200 bg-white shadow-sm transition-all duration-200 hover:border-zinc-300 hover:shadow-md dark:border-zinc-800 dark:bg-zinc-900 dark:hover:border-zinc-700 ${viewMode === 'grid' ? 'flex-col items-start gap-3 p-3' : 'items-center p-3'}`}
    >
      <div
        className={`relative flex shrink-0 items-center justify-center rounded-lg bg-zinc-100/80 text-zinc-600 transition-colors group-hover:bg-zinc-200/80 dark:bg-zinc-800/80 dark:text-zinc-400 dark:group-hover:bg-zinc-700/80 ${viewMode === 'grid' ? 'h-16 w-16' : 'h-10 w-10'}`}
      >
        <FiFolder className={`${viewMode === 'grid' ? 'h-8 w-8' : 'h-5 w-5'}`} />
        {folder.isFavorite && (
      ...
      <div
        className={`flex w-full items-start justify-between ${viewMode === 'grid' ? '' : 'ml-3 flex-1'}`}
      >
        ...
        <div className="relative flex shrink-0 flex-col items-end" ref={menuRef}>
          <button
          ...
          {showMenu && (
            <div
              className={`absolute right-0 z-20 min-w-36 overflow-hidden rounded-xl border border-zinc-200/50 bg-white/80 p-1 shadow-lg backdrop-blur-md outline-none dark:border-zinc-800/50 dark:bg-zinc-900/80 ${
                menuPosition === 'top'
